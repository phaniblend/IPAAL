...
const NODES = [
  { id: "intro", type: "reveal", phase: "Problem", content: { tag: "PROBLEM #96 (Angular)", title: TITLE, body: "Design a DataTable component API in Angular: @Input() data: T[]; @Input() columns: { key: keyof T; header: string }[]; support sorting (sortBy, sortDir signals), optional pagination (page, pageSize), and use *ngFor with trackBy for rows.", usecase: "Angular DataTable uses @Input() for data and columns, signals for sort/page state, and *ngFor for rendering." } },
...
export default createINPACTEngine({ NODES, sideItems, problemNum: 96, title: TITLE, shortName: "A — DATA TABLE API" });
