...
TypeScript/React — React.FC<Props> without using props in this step (critical):
- When the step asks to annotate a component as \`React.FC<SomeProps>\` (or \`FC<SomeProps>\`) but does NOT ask to use, display, destructure, or read props in the JSX/body, then \`const Name: React.FC<SomeProps> = () => { return <h1>...</h1>; }\` with **no** function parameter is **correct**. TypeScript still applies the props type to the component; omitting the parameter is valid when the implementation ignores props.
- Do NOT mark partial or wrong for "missing props parameter" or "add (props: SomeProps) or destructure" unless this step's instruction or success criteria explicitly require using those props (e.g. rendering \`{name}\`, implementing handlers that use props). Real syntax errors (e.g. \`ReactFC\` without a dot, wrong generic) still fail as usual.
