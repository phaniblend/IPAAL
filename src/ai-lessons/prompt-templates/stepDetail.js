...
- analogousExample: MUST show ONLY what this step asks for (one micro-step). Valid, syntactically complete code in {{LANGUAGE}}/{{FILE_MODE}}. For TypeScript/TSX: use TypeScript syntax (e.g. return type : JSX.Element, useState<number>(0), typed params). For JavaScript: no type annotations. Do not show code that implements a later step. E.g. if this step is "Create a new API service using createApi" and the next step is "Define baseQuery and empty endpoints", the example must be something like \`const api = createApi({});\` — not the full config. You may add one short // comment, then the code. Function declarations must include a body. Never output incomplete snippets. No prose.
