...
- analogousExample: MUST show ONLY what this step asks for (one micro-step). Teach the **same language constructs and goal** as the instruction (e.g. if the step is about returning a Promise from a callback, show returning a Promise that resolves; if the step is about console.log, show console.log—the **task** is logging, not the exact string; if the step is React.FC<SomeProps> with a typed JSX return, show that pattern). **Identifiers and literals may differ** from the instruction—use analogous names and content (e.g. another component name, "hi" instead of "hello") so the example illustrates the mechanism without duplicating the exact answer. Do **not** substitute a different mechanism (e.g. a plain div when the step requires a heading element pattern, or sync code when the step requires async/Promise). Valid, syntactically complete code in {{LANGUAGE}}/{{FILE_MODE}}. For TypeScript/TSX: use TypeScript syntax. One short // comment, then the code. Never output incomplete snippets. No prose.
