...
export default function RequireRole({ roles = [], children }) {
  ...
  if (roles.length > 0 && !hasRole(session, roles)) {
    return (
      <div style={{ padding: 40, maxWidth: 480, fontFamily: "-apple-system, sans-serif" }}>
        <h1 style={{ fontSize: 20 }}>Not authorized</h1>
        <p style={{ color: "#64748b" }}>
          Signed in as <strong>{session.name}</strong>, but this screen needs one of: <code>{roles.join(", ")}</code>.
