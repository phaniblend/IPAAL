...
| **recursion** | Solve a smaller version of the same problem | Base case + smaller-problem thinking |
...
- **Scale-problem:** “With 10,000 numbers, checking every pair is too many.”  
...
- **Scale-problem:** “Trying every possible start and end is too slow.”  
...
- **Scale-problem:** “We need to do it in one pass without copying the whole list.”  
...
- **Scale-problem:** “We need a rule that works for any size tree.”  
...
- **Scale-problem:** "We need a way to mark what we've already visited."
...
- **Scale-problem:** "We need a rule that works for any n without writing all the steps."
- **Concept-bridge:** "**Solve a smaller version of the same problem** — base case (0! = 1, fib(0)=0, fib(1)=1) and recursive case (n! = n × (n-1)!, fib(n) = fib(n-1) + fib(n-2))."
...
- **Scale-problem:** "We need to avoid recomputing; remember answers to smaller subproblems."
...
- `problem`, `example`, `reasoning`, `dryRun` — unchanged  
- `flowchart` — unchanged  
